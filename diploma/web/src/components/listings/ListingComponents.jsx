      ...
      <div className="details-layout details-layout-stacked"><div className="details-content">
        ...
        <div className="details-overview-row details-overview-row-split">
          ...
          <div className="details-side-stack">
            {!hideActions && <div className="details-owner-card details-owner-glass glass"><div className="details-owner-bar"><div className="details-owner-identity"><OwnerAvatar name={ad?.ownerName || ad?.userFullName || "Владелец"} avatarUrl={ad?.ownerAvatarUrl} /><div className="details-owner-copy"><div className="details-owner-name-row"><strong>{ad?.ownerName || ad?.userFullName || "Владелец"}</strong><VerificationBadge status={ad?.ownerVerificationStatus} /></div><div className="details-owner-rating-line"><span className="details-owner-rating-star">★</span><span className="details-owner-rating-score">{Number(ad?.ownerRating || 0).toFixed(1)}</span><span className="details-owner-rating-separator">•</span><span className="details-owner-rating-count">{`${Number(ad?.ownerReviewsCount || 0)} ${formatReviewWord(ad?.ownerReviewsCount || 0)}`}</span></div></div></div><button className="primary-button" type="button" onClick={() => { onOpenDialog(ad); onClose(); }}>Написать</button><button className="secondary-button" type="button" onClick={() => onOpenOwnerProfile?.(ad?.ownerId)}>Профиль</button></div></div>}
