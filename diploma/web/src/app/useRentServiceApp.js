...
export function useRentServiceApp() {
    ...
    useEffect(() => {
        ...
        async function hydrateDialogAvatar() {
            try {
                ...
                // Keep the existing fallback avatar if ad details cannot be loaded here.
            }
        }
    ...
    useEffect(() => {
        if (!profile?.id || !storage.getToken()) {
            return undefined;
        }
        ...
        const connect = () => {
            ...
            const socket = messagesSocketApi.connect();
            if (!socket) {
                return;
            }
            ...
            socket.onclose = () => {
                ...
                messageSocketReconnectRef.current = window.setTimeout(() => {
                    connect();
                }, 2000);
            };
        };
        ...
        return () => {
            ...
            if (messageSocketReconnectRef.current) {
                window.clearTimeout(messageSocketReconnectRef.current);
        ...
        const interval = window.setInterval(async () => {
            try {
                ...
                if (status?.status === "completed" && status.auth?.token) {
                    storage.setToken(status.auth.token);
                    const me = await authApi.me();
                    setProfile(me);
                    setTelegramAuth(null);
                    await refreshAll(me);
                } else if (status?.status === "expired") {
                    setError("Срок подтверждения через Telegram истек. Запросите новую ссылку.");
                    setTelegramAuth(null);
                }
            } catch (err) {
    ...
    // Восстановление пароля через Telegram без отдельного окна.
    ...
    function handleResetCodeChange(index, value) {
        if (value.length > 1) {
            ...
            document.getElementById(`reset-code-${Math.min(digits.length, 5)}`)?.focus();
            return;
        }
    ...
    async function handleDeletePayoutDetails() {
        try {
            ...
            setBusy("delete-payout-details", false);
        }
    }
